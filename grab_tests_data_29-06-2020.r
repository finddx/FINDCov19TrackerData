...
fetch_from_json <- function(url, cumul, new) {
  ...
  if (!is.na(cumul)) {
    tests_cumulative <- as.numeric(getData_json[[cumul]])
  }
  if (!is.na(new)) {
    new_tests <- as.numeric(getData_json[[new]])
  }
  return(c(new_tests,tests_cumulative))
}

fetch_from_jsonstat <- function(url, cumul, new) {
  message(url)
  tests_cumulative <- NA
  new_tests <- NA
  
  getData_json <- fromJSONstat(readLines(url))
  
  if (!is.na(cumul)) {
    tests_cumulative <- as.numeric(getData_json[[cumul]]$value)
  }
  if (!is.na(new)) {
    new_tests <- as.numeric(getData_json[[new]]$value)
  }
  return(c(new_tests,tests_cumulative))
}
...
fetch_from_selenium <- function(country, pattern) {
  ...
  line <- grep(paste0("echo: ",country,";"),readLines(paste0("output_selenium_",today_str,".txt"),encoding = "UTF-8"),value = T)
  message(line)
  content <- gsub(paste0("echo: ",country,";"),"",line)
...
filename <- paste0(DB_dir,'coronavirus_tests_',yesterday_str,'_sources_SO.csv')
...
lapply(1:nrow(info),function(r){
  ...
  if (length(data)>1) {
    ...
    nr <<- nr+1
    new_data[nr, ] <<- c(NA,country,NA,data[1],data[2],info[[r,"jhu_ID"]],info[[r,"source"]],NA)
  }
})
